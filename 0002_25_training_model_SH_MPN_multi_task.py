...
path_addr = r'G:\MPN\Phase1_module_2\Try_fliter_only_data2048\Scaled_raw_512'
file_all = []
for root_i ,dirs_i,files_i in os.walk(path_addr):
    for f_i in files_i:
        file_all.append(os.path.join(root_i, f_i))

temp_addr = np.array(file_all)
X_Y_ALL = pd.DataFrame(np.transpose( temp_addr))
X_Y_ALL.columns = ['File_addr']

X_Y_ALL["flag"] = X_Y_ALL["File_addr"].apply(lambda x:re.search("ET",x) is None)
X_Y_ALL = X_Y_ALL[X_Y_ALL["flag"]==True]
X_Y_ALL = X_Y_ALL.reset_index(drop=True)

# X_Y_ALL['GT_tiles_level'] = 0
# X_Y_ALL['Data_set'] = 'Train'
# for i in range(X_Y_ALL.shape[0]):
#     if i % 10000==0 : print(i)
#     if re.search('Temp_Scale_summary_tiles', X_Y_ALL['File_addr'][i]) is not None:
#         X_Y_ALL.loc[i,'Data_set'] = 'Del'
#     else:
#         # if re.search('Train', X_Y_ALL['File_addr'][i]) is not None:
#         #     X_Y_ALL.loc[i,'Data_set'] = 'Train'
#         # elif re.search('Test', X_Y_ALL['File_addr'][i]) is not None:
#         #     X_Y_ALL.loc[i,'Data_set'] = 'Test'
#         # else:
#         #     X_Y_ALL.loc[i,'Data_set'] = 'Valid' 
#         if re.search('Case', X_Y_ALL['File_addr'][i]) is not None:
#             X_Y_ALL.loc[i,'GT_tiles_level'] = 1
#         elif re.search('Control', X_Y_ALL['File_addr'][i]) is not None:
#             X_Y_ALL.loc[i,'GT_tiles_level'] = 0
#         else:
#             X_Y_ALL.loc[i,'GT_tiles_level'] = 2 
X_Y_ALL['Sample_id'] = X_Y_ALL['File_addr'].apply(extract_num)
            
GT_res = pd.read_csv(r'G:\MPN\Phase1_module_2\Data\Ground_true_phase1.csv')

Final_res_all =pd.merge(X_Y_ALL,
                          GT_res.loc[:,['Data_source', 
                                        'unique_sample_id',
                                        "GT"]],  
                          how = 'left',
                          left_on='Sample_id', 
...
# Final_res_all['flag'] = Final_res_all["GT"].apply(lambda x:x in ["MF",  "PV" ])


# ##
# aaaa = Final_res_all[Final_res_all['flag']==False]
# aaaa["GT"].value_counts()
# ## Final_res_all["GT"].value_counts()
# aaaa = Final_res_all[Final_res_all['flag']==True]
# aaaa["GT"].value_counts()
# Final_res_all = Final_res_all[Final_res_all['GT']!='Other']
Final_res_all = Final_res_all[Final_res_all["flag"]==True]
Final_res_all = Final_res_all.reset_index(drop=True)
Final_res_all['GT_tiles_level'] = 0
for i in range(Final_res_all.shape[0]):
    if Final_res_all.loc[i,'GT'] in ["health","Other"]:
        Final_res_all.loc[i,'GT_tiles_level'] = 0
    elif Final_res_all.loc[i,'GT'] =="MF":
        Final_res_all.loc[i,'GT_tiles_level'] = 1
    elif Final_res_all.loc[i,'GT'] =="PV":
        Final_res_all.loc[i,'GT_tiles_level'] = 2
    else:
        Final_res_all.loc[i,'GT_tiles_level'] = np.nan
 
# Final_res_all['GT_tiles_level'].value_counts()
# Final_res_all['GT'].value_counts()
...
ET_file = get_filelist(path=r"G:\MPN\Phase1_module_2\Try_fliter_only_data2048\Scaled_raw",
                        regex_= "ET")

ET_file.columns = ["File_addr"]
 
def get_ET_sample(x):
    temp_add0 = os.path.split(x)
    temp_add1 = temp_add0[-1]
    temp_add2 = temp_add1.split("_num_")
    return temp_add2[0]
     
def get_sourcr(x):
    if re.search("Test",x) is not None:
        return "Test"
    elif re.search("Train",x) is not None:
        return "Train"
    elif re.search("Valid",x) is not None:
        return "Valid"
    else:
        return "error"
ET_file['Sample_id'] = ET_file["File_addr"].apply(get_ET_sample)
ET_file['Data_source'] = ET_file["File_addr"].apply(get_sourcr) 
ET_file["GT_tiles_level"] =3
...
Final_res_all_new = pd.concat((Final_res_all.loc[:,['File_addr', 'GT_tiles_level', 
                                                    'Sample_id',"Data_source"]],
                               ET_file),axis=0)


# Final_res_all_new["GT_tiles_level"].value_counts(dropna=False)

# Final_res_all["GT"].value_counts()
Final_res_all_new = Final_res_all_new[np.isnan(Final_res_all_new["GT_tiles_level"])==False ]

Final_res_all_new["GT_tiles_level"] = Final_res_all_new["GT_tiles_level"].astype("int")
...
# Train_frame["GT_tiles_level"].value_counts()
...
torch.save(model_ss,
           r'G:\MPN\Phase1_module_2\512\SH_MF_PV_ET_25_times.pkl')

his_csv_out = clean_hist(hist_final)
his_csv_out.to_csv(r'G:\MPN\Phase1_module_2\512\SH_MF_PV_ET_25_times.csv')
