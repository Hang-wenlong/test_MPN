...
class MnistBags(data_utils.Dataset):
    ...
    def _form_bags(self):
        if self.train:
            train_loader = data_utils.DataLoader(datasets.MNIST('D:/论文code/MPN/AttentionDeepMIL/datasets',
                                                                train=True,
                                                                download=False,
            ...
            while valid_bags_counter < self.num_bag: ##共计取num-bag次
                bag_length = np.int(self.r.normal(self.mean_bag_length, self.var_bag_length, 1))
                if bag_length < 1:
                    bag_length = 1
                indices = torch.LongTensor(self.r.randint(0, self.num_in_train, bag_length))  ##随机取bag_length的索引
                labels_in_bag = labels[indices] ##取bag_length对应的标签

                if (self.target_number in labels_in_bag) and (label_of_last_bag == 0):
                    labels_in_bag = labels_in_bag >= self.target_number
                    labels_list.append(labels_in_bag)  ##吧一包数据的每个数据的标签放进去
                    bags_list.append(numbers[indices])  ##吧一包数据放进去
                    label_of_last_bag = 1
                    valid_bags_counter += 1  ##有效包的个数
            ...
            test_loader = data_utils.DataLoader(datasets.MNIST('D:/论文code/MPN/AttentionDeepMIL/datasets',
                                                               train=False,
                                                               download=False,
                                                               transform=transforms.Compose([
                                                                    transforms.ToTensor(),
